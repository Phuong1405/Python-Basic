    ...
    try:
        ...
        for line in file:
            ...
            if len(line) > 0 and line[0] == "#":
                continue
            ##gives a list of network names into list
            fields = line.split(";")

            ##if it has one name so it will print the Fatal Error line
            if len(fields) < 2:
                print(f"Fatal Error: line '{line}' has too few fields.")
                return None

            ##the loop name goes in the fields, condition makes sure the name is in alphabet
            for name in fields:
                if not name.isalpha():
                    print(f"Fatal Error: '{name}' is not a valid name.")
                    return None

            ## "who" variable is the first name in the list
            who = fields[0]

            ## the fields will be from index 1 to the last
            ## if a name of a friend in the field, it will be "who"'s friend, so it will start from the index 1
...
def add_friendship(network, name1, name2):
    ...
    #       already known to be friends, nothing needs to be done.
        ##adding name1 and name2 in to the dictionary "network"
        ##if name1 and name2 are in the dictionary, do nothing
...
def print_command(network, namelist):
    ...
    :return: None
    """
    ##condition if going through all the namelist (list), then print the Error message
    if len(namelist) != 0:
