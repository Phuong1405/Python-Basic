...
COMP.CS.100 Ohjelmointi 1
            Programming 1
Team member1:
- FirstName Lastname: VU TU PHUONG NGUYEN
- Student ID: 150360085
- email: phuong.nguyen@tuni.fi

Team member2:
- FirstName Lastname: Johnson Apanishile
- Student ID: 150421047
- email: johnson.apanishile@tuni.fi

This program collects a set of names which forms a network group of friends
Using a data structure to store the name1: key and friends: values.
Program collects user input to Print, Add , print common friends.
One person (A) can have list of a friend. And it will be the same as the other person (B), in some cases,
there will have the common friends of A and B
"""
...
def print_command(network, namelist):
    ...
    for keyname in sorted(network):
        print(keyname)
        for keynameFr in sorted(network[keyname]):
            print(f"- {keynameFr}")
...
def friends_command(network, namelist):
    ...
    for namefriend in sorted(network[name]):
        print(namefriend)
...
def common_friends_command(network, namelist):
    ...
    for friendname in network[name1]:
        if friendname in network[name2]:
            common_friends.append(friendname)
    ...
    for friendname in sorted(common_friends):
        print(friendname)
